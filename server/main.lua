...
-- TableHasValue işlevi (liste kontrolü için)
...
AddEventHandler('ak4y-advancedFishing:addItem', function(item)
    ...
    if lastTriggerTime[src] and currentTime - lastTriggerTime[src] < 9000 then
        exports["anticheatname"]:fg_BanPlayer(
            xPlayer.PlayerData.source,
            "9 saniyeden önce balık tutmaya çalıştığı için banlandı.",
            true
        )
        return
    end

    lastTriggerTime[src] = currentTime

    if not TableHasValue(allowedItems, item) then
        exports["anticheatname"]:fg_BanPlayer(
            xPlayer.PlayerData.source,
            "Geçersiz item eklemeye çalıştığı için banlandı: " .. item,
            true
        )
        return
    end
